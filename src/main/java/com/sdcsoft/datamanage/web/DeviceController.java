...
public class DeviceController {
    ...
    public Result getDeviceListByConditionAndPage(Device device, int pageNum, int pageSize) {
        ...
        List<Device> list=deviceService.getDeviceListByCondition(device);
       for(Device d:list){
           if(deviceEhcacheUtil.getCacheData(d.getDeviceSuffix())){
               d.setOnlineStates(1);//1代表  该设备在线
           }else{
               d.setOnlineStates(0);
           }
        }
    ...
    public Result getDeviceListByEnterpriseIdAndPage(Device device, int pageNum, int pageSize) {
        PageHelper.startPage(pageNum, pageSize);
        List<Device> list= deviceService.getDeviceListByEnterpriseId(device);
        for(Device d:list){
            if(deviceEhcacheUtil.getCacheData(d.getDeviceSuffix())){
                d.setOnlineStates(1);//1代表  该设备在线
            }else{
